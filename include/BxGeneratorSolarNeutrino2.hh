...
#ifndef _BXGENERATORSolarNeutrino2_HH
#define _BXGENERATORSolarNeutrino2_HH
...
//---------------------------------------------------------------------------//
/**Solar Neutrino generator: generates solar neutrino spectra, including distortions due to oscillations; stores information only about recoiled electron and interacting neutrino (slower than BxGeneratorSolarNeutrino)
*/
class BxGeneratorSolarNeutrino2 : public BxVGenerator {
public:

  ///default constructor
  BxGeneratorSolarNeutrino2();

  ///destructor
  virtual ~BxGeneratorSolarNeutrino2();
  ...
  void SetPosRot1(G4ThreeVector rot) { fSPSPos->SetPosRot1(rot); }
   ...
   G4double GetDM2()                  { return DM2 ;}
   
   void     SetTG2T(G4double val)     { TG2T = val;}
   G4double GetTG2T()                 { return TG2T ;}
  
   void     SetBinning(G4int k)       { fNumberOfSteps = k;}
   G4int    GetBinning()              { return fNumberOfSteps ;}
...
private:
  ...
  G4double BE7NuSpectrum (G4double ) ;
  G4double HEPNuSpectrum (G4double ) ;

  G4double  Interpolate(G4double, G4double, G4double, G4double, G4double) ;
  
  G4double fMinus(G4double , G4double );
  G4double fPlus(G4double , G4double );
  G4double fPlusMinus(G4double , G4double );

  BxGeneratorSolarNeutrino2Messenger*   fTheMessenger;
  ...
  G4double                     AlphaPee[11];
  G4double                     BetaPee[11];
  G4double                     NuEndPoint[11];
  ...
  G4bool                       is_read;
  G4bool                       fOldSigma;
  vector<G4double>             fNuEnergy;
  vector<G4double>             fNuProbability ;
