...
BxGeneratorSolarNeutrinoMessenger::BxGeneratorSolarNeutrinoMessenger(BxGeneratorSolarNeutrino* gen){
  generator = gen;
  fDirectory = new G4UIdirectory("/bx/generator/snu/");
  fDirectory->SetGuidance("Control of BxSolarNeutrino event generator");


  fPositionCmd = new G4UIcmdWith3VectorAndUnit("/bx/generator/snu/position",this);
  ...
  fSphereBulkCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/snu/sphere_radius",this);
  ...
  fSphereBulkRadMinCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/snu/sphere_radius_min",this);
  ...
  fSphereSurfCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/snu/surface_radius",this);
  ...
  fSphereCentreCmd = new G4UIcmdWith3VectorAndUnit("/bx/generator/snu/sphere_origin",this);
  ...
  fVesselCmd = new G4UIcmdWithABool("/bx/generator/snu/vessel",this);
  fVesselCmd->SetGuidance("Set events on the vessel inner surface");

  fOldSigmaCmd = new G4UIcmdWithABool("/bx/generator/snu/oldsigma",this);
  fOldSigmaCmd->SetGuidance("Set tree level sigma in snu");
 
 fBulkCmd = new G4UIcmdWithABool("/bx/generator/snu/bulk",this);
  fBulkCmd->SetGuidance("Set events on the vessel inner surface");

  fBufferCmd = new G4UIcmdWithABool("/bx/generator/snu/buffer",this);
  fBufferCmd->SetGuidance("Set events on the vessel inner surface");
 
  fDirectionCmd = new G4UIcmdWith3Vector("/bx/generator/snu/direction",this);
  fDirectionCmd->SetGuidance("Set the gun direction");
  

  // confine to volume
  fConfineCmd = new G4UIcmdWithAString("/bx/generator/snu/confine",this);
  ...
  fNeutrinoCmd = new G4UIcmdWithAString("/bx/generator/snu/source",this);
  G4String candidates = "Be7 pep pp CNO B8 N13 O15 F17";  
  fNeutrinoCmd->SetCandidates(candidates);
 
  fDM2Cmd  =  new G4UIcmdWithADouble("/bx/generator/snu/deltam2",this);
  
  fTG2TCmd =  new G4UIcmdWithADouble("/bx/generator/snu/tan2t",this);
  
  fNumberOfStepCmd =  new G4UIcmdWithAnInteger("/bx/generator/snu/stepnumber",this);
}


BxGeneratorSolarNeutrinoMessenger::~BxGeneratorSolarNeutrinoMessenger() {
...
void BxGeneratorSolarNeutrinoMessenger::SetNewValue(G4UIcommand* cmd, G4String newValue) { 
   if (cmd == fPositionCmd) {         
	 ...
	 BxOutputVertex::Get()->SetRagMax(BxReadParameters::Get()->GetZoneIExternalRadius() - 0.001*mm);
	   ...
	   if(newValue == "pp") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::pp);
	   } else if(newValue == "Be7") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::be7);
	   } else if(newValue == "B8") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::b8);
	   } else if(newValue == "N13") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::n13);
	   } else if(newValue == "O15") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::o15);
	   } else if(newValue == "F17") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::f17);
	   } else if(newValue == "pep") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::pep);
	   } else if(newValue == "CNO") {
		   generator->SetNeutrinoType(BxGeneratorSolarNeutrino::cno);
	   }
   } else if (cmd == fDM2Cmd){
