...
BxGeneratorAmBeSourceMessenger::BxGeneratorAmBeSourceMessenger(BxGeneratorAmBeSource* gen){
  generator = gen;
  fDirectory = new G4UIdirectory("/bx/generator/AmBe/");
  fDirectory->SetGuidance("Control of BxAmBeSource event generator");


  fPositionCmd = new G4UIcmdWith3VectorAndUnit("/bx/generator/AmBe/position",this);
  ...
  fSphereBulkCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/AmBe/sphere_radius",this);
  ...
  fSphereBulkRadMinCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/AmBe/sphere_radius_min",this);
  ...
  fSphereSurfCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/AmBe/surface_radius",this);
  ...
  fSphereCentreCmd = new G4UIcmdWith3VectorAndUnit("/bx/generator/AmBe/sphere_origin",this);
  ...
  fVesselCmd = new G4UIcmdWithABool("/bx/generator/AmBe/vessel",this);
  fVesselCmd->SetGuidance("Set events on the vessel inner surface");

  fBulkCmd = new G4UIcmdWithABool("/bx/generator/AmBe/bulk",this);
  fBulkCmd->SetGuidance("Set events on the vessel inner surface");

  fBufferCmd = new G4UIcmdWithABool("/bx/generator/AmBe/buffer",this);
  fBufferCmd->SetGuidance("Set events on the vessel inner surface");
 

  // confine to volume
  fConfineCmd = new G4UIcmdWithAString("/bx/generator/AmBe/confine",this);
  ...
  fNeutrinoCmd = new G4UIcmdWithAString("/bx/generator/AmBe/source",this);
...
BxGeneratorAmBeSourceMessenger::~BxGeneratorAmBeSourceMessenger() {
...
void BxGeneratorAmBeSourceMessenger::SetNewValue(G4UIcommand* cmd, G4String newValue) { 
   if (cmd == fPositionCmd) {         
...
G4cout<<"IN MESSENGER "<<fSphereCentreCmd->ConvertToDimensioned3Vector(newValue).z()<<G4endl;
	 ...
	 BxOutputVertex::Get()->SetRagMax(BxReadParameters::Get()->GetZoneIIIExternalRadius());
	   ...
	   BxLog(routine) << "AmBeSource source: " << newValue << endlog ;
	   if(newValue == "all") {
		   generator->SetNeutrinoType(BxGeneratorAmBeSource::AllPN);
	   } else if(newValue == "neutron0G") {
		   generator->SetNeutrinoType(BxGeneratorAmBeSource::NeutronNoG);
	   } else if(newValue == "neutron1G") {
		   generator->SetNeutrinoType(BxGeneratorAmBeSource::Neutron1G);
	   } else if(newValue == "neutron2G") {
		   generator->SetNeutrinoType(BxGeneratorAmBeSource::Neutron2G);
	   } 
   }   
}
