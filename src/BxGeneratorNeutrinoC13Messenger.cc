...
BxGeneratorNeutrinoC13Messenger::BxGeneratorNeutrinoC13Messenger(BxGeneratorNeutrinoC13* gen){
  generator = gen;
  fDirectory = new G4UIdirectory("/bx/generator/NeutrinoC13/");
  fDirectory->SetGuidance("Control of BxNeutrinoC13 event generator");


  fPositionCmd = new G4UIcmdWith3VectorAndUnit("/bx/generator/NeutrinoC13/position",this);
  ...
  fSphereBulkCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/NeutrinoC13/sphere_radius",this);
  ...
  fSphereBulkRadMinCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/NeutrinoC13/sphere_radius_min",this);
  ...
  fSphereSurfCmd = new G4UIcmdWithADoubleAndUnit("/bx/generator/NeutrinoC13/surface_radius",this);
  ...
  fSphereCentreCmd = new G4UIcmdWith3VectorAndUnit("/bx/generator/NeutrinoC13/sphere_origin",this);
  ...
  fVesselCmd = new G4UIcmdWithABool("/bx/generator/NeutrinoC13/vessel",this);
  fVesselCmd->SetGuidance("Set events on the vessel inner surface");

  fBulkCmd = new G4UIcmdWithABool("/bx/generator/NeutrinoC13/bulk",this);
  fBulkCmd->SetGuidance("Set events on the vessel inner surface");

  fBufferCmd = new G4UIcmdWithABool("/bx/generator/NeutrinoC13/buffer",this);
  fBufferCmd->SetGuidance("Set events on the vessel inner surface");
 

  // confine to volume
  fConfineCmd = new G4UIcmdWithAString("/bx/generator/NeutrinoC13/confine",this);
  ...
  fNeutrinoCmd = new G4UIcmdWithAString("/bx/generator/NeutrinoC13/source",this);
  G4String candidates = "Full First Second Flat";  
  fNeutrinoCmd->SetCandidates(candidates);
 
}


BxGeneratorNeutrinoC13Messenger::~BxGeneratorNeutrinoC13Messenger() {
...
void BxGeneratorNeutrinoC13Messenger::SetNewValue(G4UIcommand* cmd, G4String newValue) { 
   if (cmd == fPositionCmd) {         
	 ...
	 BxOutputVertex::Get()->SetRagMax(BxReadParameters::Get()->GetZoneIIIExternalRadius());
	   ...
	   BxLog(routine) << "NeutrinoC13 source: " << newValue << endlog ;
	   if(newValue == "Full") {
		   generator->SetNeutrinoType(BxGeneratorNeutrinoC13::FullN);
	   } else if(newValue == "First") {
		   generator->SetNeutrinoType(BxGeneratorNeutrinoC13::FirstN);
	   } else if(newValue == "Second") {
		   generator->SetNeutrinoType(BxGeneratorNeutrinoC13::SecondN);
	   } else if(newValue == "Flat") {
		   generator->SetNeutrinoType(BxGeneratorNeutrinoC13::FlatN);
	   } 
   }   
}
